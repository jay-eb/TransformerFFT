...
class MixAttention(nn.Module):
    def __init__(self, model_dim, atten_dim, head_num, dropout, residual):
        ...
        self.W_Q_period = nn.Linear(model_dim, self.atten_dim * self.head_num, bias=True)
        ...
        self.W_K_period = nn.Linear(model_dim, self.atten_dim * self.head_num, bias=True)
    ...
    def forward(self, Q_data, Q_period, Q_time, K_data, K_period, K_time, V_time):
        residual = Q_data.clone()

        Q_data = self.W_Q_data(Q_data).view(Q_data.size(0), Q_data.size(1), self.head_num, self.atten_dim)
        Q_period = self.W_Q_period(Q_period).view(Q_period.size(0), Q_period.size(1), self.head_num, self.atten_dim)
        ...
        K_period = self.W_K_period(K_period).view(K_period.size(0), K_period.size(1), self.head_num, self.atten_dim)
        ...
        Q_data, Q_period, Q_time = Q_data.transpose(1, 2), Q_period.transpose(1, 2), Q_time.transpose(1, 2)
        K_data, K_period, K_time = K_data.transpose(1, 2), K_period.transpose(1, 2), K_time.transpose(1, 2)
        V_time = V_time.transpose(1, 2)

        scores_data = torch.matmul(Q_data, K_data.transpose(-1, -2)) / np.sqrt(self.atten_dim)
        scores_period = torch.matmul(Q_period, Q_period.transpose(-1, -2)) / np.sqrt(self.atten_dim)
        scores_time = torch.matmul(Q_time, K_time.transpose(-1, -2)) / np.sqrt(self.atten_dim)
        attn = nn.Softmax(dim=-1)(scores_data + scores_period + scores_time)
...
class MixAttention2(nn.Module):
    def __init__(self, model_dim, atten_dim, head_num, dropout, residual):
        super(MixAttention2, self).__init__()
        self.atten_dim = atten_dim
        self.head_num = head_num
        self.residual = residual
